...
bool GraphAlgorithms::relax(Edge<LocationInfo> *edge, bool isDriving) {
  ...
  double currentDist = isDriving ? edge->getOrig()->getDrivingDist()
                                 : edge->getOrig()->getWalkingDist();
  double edgeWeight =
      isDriving ? edge->getDrivingWeight() : edge->getWalkingWeight();
  double newDist = currentDist + edgeWeight;

  if (newDist < (isDriving ? edge->getDest()->getDrivingDist()
                           : edge->getDest()->getWalkingDist())) {
    if (isDriving) {
...
std::vector<LocationInfo> GraphAlgorithms::getPath(Graph<LocationInfo> *g,
                                                   ...
                                                   const int &dest,
                                                   bool isDriving) {
  if ((isDriving ? g->findVertexById(dest)->getDrivingDist()
                 : g->findVertexById(dest)->getWalkingDist()) == INF) {
...
GraphAlgorithms::getParkingNodes(
    Graph<LocationInfo> *graph, int source, const std::vector<int> &avoidNodes,
    const std::vector<std::pair<int, int>> &avoidSegments) {
...
std::vector<EnvironmentalPath> GraphAlgorithms::drivingWalkingRoute(
  ...
  auto parkingNodes = GraphAlgorithms::getParkingNodes(
      graph, source, avoidNodes, avoidSegments);
  ...
  if (!validRoutes.empty()) {
    std::sort(validRoutes.begin(), validRoutes.end(),
              [](const EnvironmentalPath &a, const EnvironmentalPath &b) {
    ...
    std::sort(approximateRoutes.begin(), approximateRoutes.end(),
              [](const EnvironmentalPath &a, const EnvironmentalPath &b) {
                if (a.totalTime != b.totalTime)
                  return a.totalTime < b.totalTime;
                return a.walkingTime > b.walkingTime;
