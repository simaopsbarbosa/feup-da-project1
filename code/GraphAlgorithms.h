 ...
 * @brief Contains graph-based algorithms for route planning, including driving, walking, and environmental routes.
 */
...
/**
  ...
  * @param isDriving Indicates whether the relaxation is for driving (true) or walking (false). Defaults to true.
...
/**
  * @brief Computes a normal route considering user constraints and optional inclusion of a specific node.
  * @param graph Pointer to the graph.
  * @param source The source node ID.
  * @param dest The destination node ID.
  * @param avoidNodes A list of node IDs to avoid.
  * @param avoidSegments A list of road segments (pairs of node IDs) to avoid.
  * @param includeNode An optional node to include in the route (-1 means no specific inclusion).
  * @return A vector of LocationInfo representing the computed route.
  */
std::vector<LocationInfo> normalRoute(Graph<LocationInfo> *graph, int source, int dest, const std::vector<int> &avoidNodes,
                                      const std::vector<std::pair<int, int>> &avoidSegments, int includeNode = -1);

/**
  * @brief Computes an environmentally-friendly route combining driving and walking.
  * @param graph Pointer to the graph.
  * @param source The source node ID.
  * @param dest The destination node ID.
  ...
  */
std::vector<EnvironmentalPath> environmentalRoute(Graph<LocationInfo> *graph, int source, int dest, double maxWalkingTime,
                                                  const std::vector<int> &avoidNodes, const std::vector<std::pair<int, int>> &avoidSegments);

/**
  * @brief Finds suitable parking nodes near the destination to optimize the environmental route.
  * @param graph Pointer to the graph.
  * @param source The source node ID.
  * @param avoidNodes A list of node IDs to avoid.
  * @param avoidSegments A list of road segments (pairs of node IDs) to avoid.
  * @return A vector of pairs, each containing a parking node and its corresponding driving path.
  */
